...
def parse_args():
    ...
    parser.add_argument("--evaluation-frequency", type=int, default=200_000,
        help="every how many timesteps to evaluate the agent. Evaluation is disabled if 0.")  # the underscore in 200_000 is a feature to improve readability of large numbers and doesn't affect the numerical value
    parser.add_argument("--evaluation-num-episodes", type=int, default=50,
        help="the number episodes to run per evaluation")
    parser.add_argument("--early-stop-window", type=int, default=3, help="Number of recent evals to use for early stopping.")
...
if __name__ == "__main__":
    ...
    # Confirm with user if save-model is not specified
    if not args.save_model:
        print("WARNING: You did not specify --save-model.")
        print("If you interrupt or crash, all training progress will be lost.")
        confirm = input("Do you want to continue without saving? [y/N]: ").strip().lower()
        if confirm not in ("y", "yes"):
            print("Exiting.")
            sys.exit(0)

    run_name = f"{args.env_id}_{args.exp_name}_{args.vlc_ckpt}"
    ...
    if args.vlm_reward_weight != 0:
        ...
        # make sure your env_id task is in the CAPTION_PATH (e.g. held-out Metaworld tasks are in mw10_split)
    ...
    for global_step in range(args.total_timesteps // NUM_TASKS):
        ...
        if args.vlm_reward_weight != 0:
            ...
            if current_t >= offset_timestep:
                ...
                if args.reward_normalization_gymnasium:
                    returns = returns * gamma * (1 - terminations) + rewards
        ...
        if global_step >= args.learning_starts:
            ...
            if total_steps % args.evaluation_frequency == 0 and global_step > 0:
                ...
                if len(recent_success_rates) >= args.early_stop_window and np.mean(recent_success_rates[-args.early_stop_window:]) > 0.95:  # if at least n evals and last n evals are >95% success
